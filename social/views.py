...
def ticket(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            message = f"{cd['name']}\n{cd['email']}\n{cd['phone']}\n\n{cd['message']}"
            #مبحث send_mail: به این صورت  است که ما تابع سند میل را مینویسیسم و سپس از سمت چپ به راست باید یک سری دیتا داخل یک پرانز ست 
            # که این دیتا ها عبارت است از اولی موضوع ایمیل دومی پیام یا مسیج ایمیل سومی ادرس ایمیل فرستنده و چهارمی ادرس ایمیل گیرنده
            send_mail(cd['subject'], message, 'matinkiaee.sy81@gmail.com', ['kiaeematin92@gmail.com'],
                      fail_silently=False)
...
def contact(request,rel,username):
